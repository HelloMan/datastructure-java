...
public class ContainerWithMostWater {
    ...
    public int maxArea(int[] height) {
        ...
        int maxArea =0;
        while (left < right) {
            System.out.println(String.format("area of two lines for [%d,%d] and [%d,%d] is %d", left, height[left], right, height[right], (Math.min(height[left], height[right]) * (right - left))));
